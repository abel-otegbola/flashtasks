...
export default function Button({ variant, className, href, size, disabled, onClick, children, ...props }: buttonProps) {
    const variants = {
        primary: "hover:bg-primary/[0.8] bg-primary text-white",
       ...
       <>
            ...
            href ? 
                <Link role="button" to={href} className={`flex items-center justify-center md:gap-2 gap-1 w-fit 
                    ...
                    ${size === "xs" ? "rounded-[2px] text-[8px] py-[2px] md:px-[8px] px-[4px]" : size === "small" ? "rounded text-[12px] py-[4px] md:px-[12px] px-[8px]" : size === "large" ? "rounded-[12px] md:py-[16px] py-[10px] md:px-[32px] px-[28px]" : "rounded-[6px] text-[14px] py-[10px] px-[18px]"} 
                    ${className} 
                     `}> 
                    { children }
                </Link>
