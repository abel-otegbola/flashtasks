...
function Slider({slides, activeSlider}: {slides: {title: string, text: string}[], activeSlider: number}) {
  return (
    <div>
        ...
        <div
            key={idx}
            className="absolute left-0 right-0 flex-shrink-0 w-full flex flex-col items-center text-center gap-2 transition duration-[1000ms] ease-in-out p-6 rounded-[10px] bg-black/20 backdrop-blur-sm border border-gray-500/[0.2]"
            style={{ 
            width: "100%", 
